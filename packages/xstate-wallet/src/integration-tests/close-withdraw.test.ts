...
it('allows for a wallet to close the ledger channel with the hub and withdraw', async () => {
  ...
  playerA.channelWallet.state.workflows[0].service.send({type: 'USER_APPROVES_CLOSE'});
  await waitForExpect(async () => {
    expect(hub.workflowState).toEqual('waitForUserApproval');
  }, 3000);
  hub.channelWallet.state.workflows[0].service.send({type: 'USER_APPROVES_CLOSE'});
